...
use crate::{
    arch::aarch64::{
        ...
        gic::vgic::{
            ...
            GICD_REG_IPRIORITYR_OFF, GICD_REG_ICENABLER_OFF, GICD_REG_ICPENDR_OFF,
        },
    },
    baocore::{
...
use super::{gicd::GicdHw, gicv3::GicrHw, vgic::vgicd_emul_handler, GIC};
...
pub fn vgic_init(vm: &mut VM, vgic_dscrp: &VGicDscr) {
    ...
    let mut vgicd = vm.arch.vgicd.inner.write();
    vgicd.int_num = unsafe { GIC.get().unwrap().max_irqs };

    vgicd.typer = ((vgicd.int_num as u32 / 32 - 1) & 0b11111) // ITLN
